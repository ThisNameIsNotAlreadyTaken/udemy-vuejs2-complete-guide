<template>
    <div class="form-group has-feedback" :class="{ 'has-success': !$v.$invalid, 'has-error': $v.$dirty && $v.$invalid }">
        ...
        <div class="col-sm-10">
            <input type="text" class="form-control" :placeholder="placeholder || name" @input="$emit('update:value', value)" v-model="value" @blur="$v.$touch()">
...
<script>
    export default {
        props: ['$v', 'name', 'value', 'errorText', 'placeholder']
    }
</script>